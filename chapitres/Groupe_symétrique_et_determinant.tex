...
\begin{prop}[Signature d'une transpositions]
    ...
    \tcblower
    Soit une transposition $\sigma=\begin{pmatrix}
        i & j
        \end{pmatrix}$, quitte à retourner la transposition on suppose $i<j$ alors $\sigma(j)=i<j=\sigma(i)$, c'est la seul inversion donc $\varepsilon(\sigma)=-1$ 
\end{prop}
