\titre{Construction de \texorpdfstring{$\mathbb{R}$}{R} et de \texorpdfstring{$\mathbb{C}$}{C}}{Suites de Cauchy, coupures de Dedekind, théorèmes fondamentaux pour l'analyse}
...
Les termes de cette suite sont successivement, en valeur approchée, 2, 1.5, 1.416667, 1.414216, 1,414213. Ils semblent être de plus en plus proches les uns des autres. On peut formaliser cette notion.
...
\subsection{Une autre construction de \texorpdfstring{$\mathbb{R}$}{R}}
\subsubsection{Les réels, version 2}

On peut aussi voir le réel $\sqrt{2}$ comme la borne supérieure de l'ensemble $\{x\in \Q | x<0 \lor x^2<2\}$, ou la borne inférieure de l'ensemble $\{x\in \Q | x>0 \land x^2>2\}$.
...
\begin{defini}[Addition]
    Soient $x=(A, B)$ et $y=(A', B')$ deux coupures. On définit l'ensemble $A+A'=\{x+y | (x,y) \in A \times A'\}$.
    
    Alors $x+y=(A+A', \Q\backslash A+A')$ est une coupure.
    \tcblower
    \begin{itemize}
        ...
        \item Si $\Q\backslash A$ et $\Q\backslash A'$ n'ont pas de borne inférieure alors $Q\backslash A+A'$ ne peut en avoir.
    \end{itemize}
\end{defini}

\begin{prop}[Zéro et opposé]
    Soit $0_{\R}$ la coupure associée au rationnel $0$. Pour toute coupure $x=(X, \Q\backslash X)$ il existe $-x$ il que $-x+x=0$.
    \tcblower
    
    Dans le cas où la coupure représente un rationnel $q$, il suffit de prendre la coupure assoiée à $-q$. On suppose maintenant que $X$ n'a pas de borne supérieure.

    On considère l'ensemble $-X=\{-q|q\in X\}$. Alors $(\Q\backslash -X, -X)$ est une coupure et $-X+X=0_{\R}$.  
\end{prop}

\begin{defini}[Réel positif]
...
\begin{ex}
    $\R^n$ muni de la distance euclidienne ($d(a,b)=(\sum_{k=1}^n) a_k^2)^{\frac{1}{2}}$ est complet.
    \tcblower
\end{ex}
...
\begin{exo}[Deux espaces de fonctions]
    ...
    \begin{enumerate}
        ...
        \item En utilisant le théorème de Heine et le fait que toute suite convergente de fonctions uniformément continues converge vers une fonction continue, montrer que $(\mathcal{C},d_1)$ est complet.
